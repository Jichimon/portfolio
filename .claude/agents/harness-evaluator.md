...
**That 37-turn figure holds only when every corpus the brief names arrives precomputed.** The one dispatch measured against this budget — `EVAL-001`, scoring `TASK 66` — spent it because the trace index and the work-item extract were both handed over as files, assembled by a deterministic script before this role was ever dispatched, never read raw and never assembled by this role or the orchestrator (`ADR-009`, sub-decision 3). Bootstrap item 6 now names a third possible corpus, the status-history ledger, which did not exist when the 37-turn figure was measured — the same condition applies to it: handed over precomputed, or this budget has no measurement behind it for that brief. **A brief that hands over a raw corpus instead of an extract is a different budget, not this one:** `TASK 55` measured 0 of 3 slices cut when briefed with a pre-written extract and forbidden to open the source, against 1 of 1 cut at roughly 100k tokens when briefed to go read the sources and derive the rest (`P-09`).
...
6. **The status-history ledger**, if the brief names one. It is `K2`'s whole substrate: every transition the committed register records, derived from the git history of the work-item register. **You hold no `Bash`, so you cannot derive it** — it is handed to you as a file, generated by the orchestrator before you are dispatched. If the brief does not name one, say `K2` is `unmeasurable` and why, exactly as `EVAL-001` did; do not infer it from the register, which carries current status only.
