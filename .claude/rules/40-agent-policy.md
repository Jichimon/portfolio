...
| **G-06** | **Every run carries a budget**, and exhaustion terminates it `FAILED` with `reason: budget_exhausted` and the budget named — never a partial success. `maxTurns` is **enforced** natively; `maxToolCalls`, `maxRuntime` and `maxRetries` are **observed** from the trace at wrap-up; `maxCost` is **not available** and is never reported as a number. | 1 for `maxTurns` · 2 for the observed three | INC-06 · A8 |
