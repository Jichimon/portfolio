...
| E2E runner, and what "real" means for this stack (real browser, real build, real filesystem) | Playwright. "Real" is Astro's own documented pattern: `npm run build` then `webServer` serves the actual output via `npm run preview` — never a mock, matching `T-02` by construction. **Chromium only, in the CI-blocking gate** — amended 2026-09-01 (`ADR-006`), on a real monitored CI run that caught `e2e smoke` still in flight at 89 minutes while `mutation` had not even started, the opposite of every prior local-only estimate. Firefox/WebKit are a stated, accepted gap (`T-05`, `C-11`), restorable in `playwright.config.ts`'s `projects` array the day a real cross-engine defect motivates it |
