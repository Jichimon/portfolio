...
/**
 * A config file declares; it does not act.
 *
 * TASK 89. `site/astro.config.mjs` swept stale cache directories from its MODULE BODY,
 * which meant the sweep ran in every process that loaded the config — not only
 * `astro build`, but `astro check`, `astro preview`, `vitest run` (through
 * `getViteConfig()`), and anything running inside a Stryker sandbox, whose
 * `site/node_modules` is a symlink to the real one. Demonstrated rather than argued: two
 * directories planted in the real `site/node_modules` were both deleted by a plain
 * `vitest run`. A test runner garbage-collecting a build cache is an ordering hazard,
 * and the only thing that made it possible was the config reaching for `rmSync`.
 *
 * So the property is about the FILE, not about the line: a config may not reach a
 * mutating filesystem API at all. That needs no scope analysis and no brace counting,
 * and it cannot be walked around by wrapping the call in a top-level IIFE.
 *
 * The allowlist is INVERTED on purpose — it names the read-only APIs, so an API nobody
 * thought of is a finding by default rather than a silent pass (P-13). A namespace or
 * default import is a finding for the same reason `G-13` makes a guard that cannot
 * evaluate deny: nobody can see statically what `fs.*` reaches for.
 */
