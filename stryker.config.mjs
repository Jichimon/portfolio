...
export default {
  ...
  //
  // RAISED 2026-08-30, TASK 88: 76.0 -> 77.0, against a re-measured 77.58%. Two files moved and
  // the item is the reason for both: status-history.mjs went to ZERO survivors and zero
  // uncovered (236 killed, 55 ignored), and markdown-shape.mjs entered the surface at 100% with
  // no suppression at all. Set to 77.0, not 77.58, for the same timeout-drift reason every
  // floor before it was set below its measurement: this run reported 80 timeouts, and runs on
  // this surface have reported 21, 45, 66, 72 and 85. 0.58 points of slack.
  //
  // WHAT TASK 88 DECIDED, and it is policy rather than a note about one file. D3 scoped
  // mutation away from render templates because mutating them produces equivalent mutants and
  // noise. That is right about a template's SENTENCES and wrong about its SHAPE:
  //
  //     a render template's SENTENCES are noise  -> suppressed AT THE MUTANT, with a reason
  //     a render template's SHAPE is structure   -> asserted, never suppressed
  //
  // Measured, not reasoned: renderLedger's 27 surviving CallExpression mutants each deleted a
  // push of a blank line or a prose sentence, and read as equivalent. They are not. A blank
  // line in Markdown is SYNTAX — drop the one between a paragraph and a table and the table
  // stops rendering — and the ledger's reader is harness-evaluator, which reads it as a
  // Markdown document. They survived because every assertion on that artifact matched content
  // with `[\s\S]*` and none asked whether the document was well-formed. scripts/guards/lib/
  // markdown-shape.mjs is that missing assertion, and it quotes no sentence, so prose stays
  // free to change without touching a test. Every future lib/ module that renders gets the
  // same treatment: it stays IN this surface.
  //
  // Two alternatives were priced and declined, recorded so nobody re-derives them. A golden-file
  // snapshot kills everything in one assertion and would retire status-history.mjs's own
  // StringLiteral suppression — declined because every prose edit then regenerates the fixture, and an artifact
  // regenerated by reflex is a control that stops being read, which is the failure
  // mutation-suppressions.mjs's own header records this repository hitting three times. And
  // excluding render templates by a `*.render.mjs` naming convention is a declared property
