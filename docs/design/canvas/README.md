...
**Relocated and unit-tested by `TASK 63`** (2026-08-28): this file carried zero test coverage of any kind before then — it sat outside both `scripts/guards/**/*.test.mjs` (the `guard tests` gate step) and Stryker's mutate glob, so a test written beside it would have run under nothing. The checkable logic now lives in `scripts/guards/lib/canvas.mjs`, in two layers:

- **Structural** (registration, derived-screen drift, in-page anchors, locale hygiene, spec agreement) — pure functions over the artboards themselves, battery-tested with real, historically-found defects planted and caught. These survive any future redesign unchanged, because they assert a *relationship* between artifacts, never a value one of them happens to hold today.
- **Declared** (the two breakpoints, the theme attribute, the chrome class names, the copy-counting vocabulary) — literals specific to the *current* design version, deliberately pulled out of the guard and into `scripts/guards/guards.config.json`'s `canvas` key rather than hardcoded in `canvas.mjs`. **A redesign that changes a breakpoint or a class name edits that config, not the guard** — the failure message on a mismatch names the exact config path to fix (e.g. `declared at scripts/guards/guards.config.json → canvas.breakpoints.medium`). Battery-tested the same way, plus a rehearsal proving the round trip: change a declared value, confirm `verify.mjs` fails naming it, revert, confirm it passes again.

This file (`verify.mjs`) still does every `readFileSync`/`readdirSync`/`JSON.parse` and reads the config; the relocated functions never touch disk, which is what makes them fast to test in isolation.
...
| Property | Scope | Layer | Why it exists |
| --- | --- | --- | --- |
| Registration is bidirectional — every `src/*.dc.html` is in `canvas.json` **and** in `local-preview.mjs`, and vice versa | all | structural | A screen that exists in `src/` and nowhere else is invisible in both the canvas and the preview, and nothing complains |
| Every derived screen matches `derive.mjs`'s current output | all | structural | A hand-edited copy of a generated file is drift nothing else would ever surface |
| Every page carries the language switcher | pages | declared — `canvas.chrome.langSwitcherClass` | Chrome belongs on every page, not on the ones somebody remembered |
| Three responsive states, and the rail actually collapses at narrow | all · pages | declared — `canvas.breakpoints`, `canvas.chrome.railStaticSelectorPattern` | The contract is the design, so a screen missing it is a screen that breaks on a phone |
| No fixed width floor survives | all | declared — `canvas.fixedWidthFloorPattern` | A `min-width: 1024px` guaranteed a horizontal scrollbar on every phone and lived on all six screens for two rounds |
| Every in-page `href="#…"` resolves to an `id` in the same file | all | structural | Found four home tiles pointing at `#experience`, a section that does not exist on home. Looking had not found it |
| No visible copy states how many of a growing thing there are | pages | declared — `canvas.growsVocabulary`, `canvas.countsVocabulary` | Found nine spots counting case studies, deep dives and employers. "Five case studies" is wrong the day a sixth lands and nothing fails when it does. Sentence-scoped, because *"Three specific problems … documented as separate case studies"* puts four words between the number and the noun |
| A Spanish screen never links into an unprefixed route | pages | structural | Found the Spanish home's wordmark still pointing at the English home. Proven in red (`P-14`) after its first version excused every `href="/"` and hid exactly that |
| The switcher marks a current locale — except on the 404, which is designed to have none | pages | declared — `canvas.chrome.currentLocaleClass`, `canvas.switcherExcluded` | A switcher with nothing current elsewhere would mean the locale was lost |
| `src/` and `docs/design/claude-design-brief.md`'s screen inventory name the same artboards, checked in both directions | all | structural | The eight checks above catch a screen that is broken; none of them catches a screen that the specification no longer knows exists, or a specification that still describes a screen nobody built. A twelfth artboard landing in `src/` and never reaching the document the implementation reads is invisible to every other check here — and so is the opposite, a screen deleted while the specification still describes it |
