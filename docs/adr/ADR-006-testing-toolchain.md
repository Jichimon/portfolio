...
## Amendment · 2026-09-01 — the e2e step was hung, not slow, and the gate now runs in profiles

**This amendment corrects the one above.** The Chromium narrowing was a real saving and a correct application of `T-05`, and it was applied to the wrong diagnosis. The preceding amendment closed with *"what is not yet known: whether Chromium alone brings the gate comfortably under a sane bound"*. The answer arrived on the next push and it was not a matter of degree: the Chromium-only run died at **exactly the same 90-minute timeout** as the three-engine run before it. A 3× cut in browser process launches that moves the wall time by nothing was never fixing the thing that was wrong.

**The real mechanism (`INC-18`).** `astro preview` starts a foreground server unless `--background` is passed **or** it detects an AI coding agent in the environment — `isRunByAgent()`, backed by the `am-i-vibing` package, whose variable list contains `CLAUDECODE` and does not contain `GITHUB_ACTIONS`. The author runs the gate inside an agent, so the preview daemonized, `execFileSync` returned, and `globalSetup` completed. On a runner, the same line blocks forever. The suite ran zero tests in all three cancelled runs; the last one's orphan-process list named `npm exec astro preview` still alive at cleanup.

**Reproduced before being fixed, and re-run after.** `env -u CLAUDECODE node node_modules/@playwright/test/cli.js test` — bounded at 180s, killed at 180s having run no test, failing at `preview-lifecycle.ts:16` inside `globalSetup`. After adding `--background`, the same command with the same environment: **171 passed in 51.7s**. That is the whole fix, and the reason it took three CI runs to find is the second half of `INC-18`: `gate.mjs` captured each step's stdout and printed it only on completion, so a step that never completed printed nothing.

**What changes in this ADR's rows.**

- **The preview lifecycle is explicit, not inferred.** `globalSetup` calls `astro preview --background`. The prior row's claim — *"this Astro version's `astro preview` is a background daemon that returns as soon as it has forked"* — described the author's machine rather than the tool, and is corrected in `30-testing.md`'s stack table as well. Owning the daemon's lifecycle in `globalSetup` rather than through Playwright's `webServer` option is unchanged and still right, for the original reason: `webServer` manages a foreground process and reads the parent's exit as a failure.
- **The e2e tier splits by tag, not by file.** The visual-capture matrix — every derived route × three widths × two themes, 102 tests — carries `@deep`. The gate runs `--grep-invert @deep` on every push (69 tests, 23.8s locally) and `--grep @deep` in the `full` profile (102 tests, 38.7s locally). 69 + 102 = 171, checked rather than assumed: the split moves tests between tiers and loses none.
- **Chromium-only stands**, on its own merits, with the correction that it was never the fix for the hang.

**The gate now has two profiles, and the reason is arithmetic.** Stryker's default concurrency is `os.availableParallelism() - 1`: 11 on the author's machine, **2** on a standard `ubuntu-latest` runner, across roughly 7,900 mutants. The local cold run measures ~10–11 minutes; no CI run has ever produced a number, because the e2e hang sat in front of it every time. **[Superseded within hours — see the measurement below: 8m11s. Kept because the reasoning it justified is the decision this amendment records, and a projection deleted is a projection nobody learns from.]** Rather than guess at a bound for a step nobody has measured on that hardware, the mutation step and the visual-capture matrix are `deep`:

```text
fast   every check except those two   -> a bare `node scripts/gate.mjs`, and every push
full   everything                     -> `--profile full`: nightly CI, on demand, and the
                                         local run that closes a work item
```

**Nothing left the mutate glob and the mutation floor did not move** (`break: 77.0`). This is a cadence decision, not a coverage one, and the distinction is kept honest mechanically rather than by intention: a deferred step is reported `DEFER` — never `SKIP`, never silence — the headline reads `GATE PASSED (profile: fast)` rather than a bare `GATE PASSED`, and the deferral line names both the steps and the profile that runs them.

**What is accepted, stated rather than left implicit (`C-11`).** Between a push and the next nightly run, a mutation regression and a visual-capture failure are not caught by CI. The mitigation is that neither is caught by a *human* faster than the nightly run either, and that both still block a work item from closing locally. The residual is real: a change merged and left alone for a day is a change whose mutation score nobody has read. That is the price of a gate that finishes, and the alternative — priced and declined this session — is a larger paid runner.

**MEASURED 2026-09-01, and the estimate above was wrong.** The first `full` CI run (`33571567866`, success in 11m8s) put `mutation` at **8m11s cold** — no incremental cache to restore — scoring 79.49 against the 77.0 floor, and `e2e visual capture` at 53.8s. The reasoning that produced *"~10-11 minutes locally, hours on a runner"* took a 5x drop in Stryker workers as a 5x drop in wall time, which a step that is not purely parallel does not obey. The tier assignment therefore stands on a real cost — ~8 minutes per push against the fast gate's 2m30s — rather than on a projection, and `TASK 111`'s entry records it as a decision the author may revisit rather than a constraint.

**Review trigger.** The first `full` CI run produces the first real numbers for both heavy steps on a 2-core runner. If mutation does not finish inside its 90-minute bound there, the decision to revisit is between a larger runner, a validated incremental cache (`stryker.config.mjs` already states what would have to be proven first), and a narrower cadence — not a lowered floor.

## Sources

Two researcher passes, this session, 2026-08-19 — the second specifically to quantify the `node:test`-vs-Vitest mutation-runner trade-off after the author questioned the first draft's unsupported claim. Official/vendor, fetched 2026-08-19: Astro docs *Testing*, *Content Loader API Reference*; Stryker Mutator docs *NodeJS guide*, *Configuration*, *Config file*, *Plugins*, *Tap Runner*, *Vitest Runner*; Stryker Mutator blog, *Announcing StrykerJS 7.0: Vitest and Tap test runner support* (published 2023-06-05, confirming `tap-runner` is not new); Stryker Mutator GitHub Releases (v10.0.0, 2026-08-14, all three packages — core, tap-runner, vitest-runner — same release day); npm registry metadata for `@stryker-mutator/core`, `@stryker-mutator/tap-runner`, `@stryker-mutator/vitest-runner`, `@playwright/test`, `cypress`. Independent: GitHub issues `stryker-js#2166` (multi-config attempt, unresolved) and `#3320` (slow-run report, no isolated cause), `stryker-net#2640` (multi-config feature request, open) — used only to confirm no multi-runner support exists, not for any performance number. Four SEO-comparison sites (bugbug.io, webfuse.com, thinksys.com, tech-insider.org) on Playwright-vs-Cypress browser-engine coverage, converging but not verified against Cypress's own docs — flagged as thin/secondary. Repository data (`D1`), read directly: `.claude/rules/30-testing.md`, `scripts/gate.mjs`, `.github/workflows/ci.yml`, then named `harness.yml` (Node 24), `progress/*.md` (the by-hand mutation-result convention), `docs/adr/ADR-002-content-pipeline.md`, confirmed no `package.json` exists anywhere yet.
