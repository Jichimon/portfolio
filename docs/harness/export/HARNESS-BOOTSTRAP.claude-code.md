...
**A sibling document exists for OpenCode.** Everything between the two shared-core markers is byte-identical in both. If you are comparing them, the diff is entirely in this section and in the installation appendix — that is by design, and a check in the originating project fails when it stops being true.

---

## 0 · How Claude Code loads this

The harness has four planes (§1). Claude Code gives each one a home, and this is the mapping the appendix installs:

| Plane | Where it lives | How it loads |
|---|---|---|
| **Adapter** | the project instruction file at the repository root | automatically, every session |
| **Rules** | one directory of rule files | automatically, every session — and **path-scoped rules load only when a matching file is touched** |
| **Procedures** | one directory per procedure, each holding a skill file | invoked by name; mark them non-model-invocable |
| **Roles** | one directory of agent files | selected by the orchestrator at delegation time |
| **Control plane** | the settings file: deny rules plus hook registrations | deny rules always; hooks on every tool call |
| **Trace** | a gitignored directory, written by hooks only | on every hook event |

Four properties of this tool that the harness depends on. **Verify each against the current documentation before relying on it** — a runtime's behaviour is a contract it owns both ends of, and this document is a claim about a version that has since moved (`P-04`):

1. **Rules load automatically and the load is observable.** That is what makes *"the agent must read the rules"* stop being a hope. It is also why rule bodies go in the rules directory and never in the adapter (invariant 1).
2. **Path-scoped rules.** A rule file may declare the paths it applies to, and it loads only when work touches one. This is what keeps the testing and implementation surfaces off the always-loaded budget without relying on anyone's discipline.
3. **A pre-tool hook can block a call**, and it runs *before* permission rules are evaluated. That is the only mechanism that survives a permissive permission mode, and it is half of what `G-03` means by a boundary. **A hook that exits with an ordinary error code does not block** — it is treated as a non-blocking hook error and the call proceeds. This is `INC-12`, and getting the exit code wrong opens every boundary at once.
4. **Subagents inherit the project rule hierarchy.** They do *not* inherit ordinary documentation, which is why every role file carries a bootstrap section naming what it must read (§13).

**Two settings are load-bearing and are set in the project's own settings file**, where a project value beats a user value:

```text
permissions.disableBypassPermissionsMode: "disable"    G-04 — enforced, not requested
disableAllHooks: false                                 the one switch that turns every guard off
```

**What this tool does not give you, stated so it is not assumed:** nothing here is an operating-system sandbox. The declared enforcement environment is `policy-controlled` (§1), and the harness claims nothing beyond it.
...
```text
<repo root>/
├── CLAUDE.md                      the adapter — identity, layout, commands, POINTERS ONLY
...
├── evidence/                      README committed; runs/ gitignored, hooks-only
├── scripts/
...
│     └── hooks/                   the single pre-tool entry point, and the trace writer
└── .claude/
   ├── rules/                      THE REGISTRY — one id space, one file per surface
   ├── agents/                     one file per role
   ├── skills/<name>/SKILL.md      one directory per procedure
   └── settings.json               wiring and permissions ONLY — no rule bodies
```
...
### 22.2 · The registry files

Six files, one id space, split by surface (§5). Two of them declare the paths they apply to; the other four load always.

```text
.claude/rules/00-hard-rules.md      H-*   always
.claude/rules/10-process.md         P-*   always
.claude/rules/20-integrity.md       C-*   always
.claude/rules/40-agent-policy.md    G-*   always
.claude/rules/30-testing.md         T-*   paths: the guards, the app, any test file
.claude/rules/50-implementation.md  S-*   paths: the app source, the root manifest
```

A path-scoped file declares its scope in frontmatter:
...
**The numbering gap between 20 and 40 is deliberate**, and so is the one at 60: leave room for a surface nobody has needed yet, so adding one never means renumbering the ones that logs already cite.

**Budget the always-loaded plane and check it.** The vendor guidance is a per-file length, not a total — so pick a total, write the number in the guard config **with the rationale beside it**, and let the check name the outstanding work rather than staying quietly green. A number chosen rather than measured says so (`C-01`).

### 22.3 · The control plane

The settings file carries wiring and permissions and nothing else. Every hard rule needs **both** halves: a deny rule *and* the hook dispatch. The deny rule is what survives a permissive mode; the hook is what can reason about a case a glob cannot express.

```json
{
  "permissions": {
    "deny": [
      "Write(./<frozen-input-dir>/**)",
      "Edit(./<frozen-input-dir>/**)",
      "NotebookEdit(./<frozen-input-dir>/**)",
      "Write(./evidence/**)",
      "Edit(./evidence/**)",
      "Write(./.git/**)",
      "Edit(./.git/**)",
    ...
    "ask": ["Bash(rm -rf:*)", "Bash(rm -fr:*)", "Bash(shred:*)", "Bash(dd:*)"],
    "disableBypassPermissionsMode": "disable"
  },
  "hooks": {
    "PreToolUse":        [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/pretooluse.mjs" }] }],
    "PostToolUse":       [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/record-event.mjs" }] }],
    "PostToolUseFailure":[{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/record-event.mjs" }] }],
    "PermissionDenied":  [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/record-event.mjs" }] }],
    "SessionStart":      [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/record-event.mjs" }] }],
    "SessionEnd":        [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/record-event.mjs" }] }],
    "SubagentStart":     [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/record-event.mjs" }] }],
    "SubagentStop":      [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/record-event.mjs" }] }],
    "InstructionsLoaded":[{ "matcher": "*", "hooks": [{ "type": "command", "command": "node scripts/guards/hooks/record-event.mjs" }] }]
  },
...
**The deny list above is longer than the rule requires, and that is the point.** `H-01` says *no version-control write*; a subcommand allowlist is how that becomes checkable. Enumerating every writing subcommand is a roster and rosters go stale (`P-13`) — so the hook does the property check, and this list is the belt to its braces.

**A check should assert that every hard rule has a deny rule behind it**, and that no boundary rests on an "ask" rule. Derive it from the registry: read the hard-rule file, find the boundaries it declares, and verify each has a matching deny entry. That is a property; a hand-kept list of expected deny strings is a roster.

### 22.4 · The single pre-tool entry point

One hook registration, dispatching internally. The skeleton, with the two things that must not be got wrong marked:

```js
#!/usr/bin/env node
// The single pre-tool entry point. Every rung-1 boundary is dispatched from here.
//
// FAIL CLOSED (G-13): any internal failure blocks, naming the reason. A guard that
// disappears when its own machinery stumbles was never a boundary (INC-12).

import { readFileSync } from 'node:fs';

const BLOCK = 2;   // the exit code the runtime treats as BLOCKING. Verify this against
                   // the current documentation — an ordinary non-zero exit does NOT block.

function deny(rule, guard, why) {
  process.stderr.write(`DENIED by ${rule} (${guard}): ${why}\n`);
  process.exit(BLOCK);
}
...
try {
  ...
  // Unparseable input is not a reason to allow. It is a reason to stop.
  deny('G-13', 'entry-point', `the hook payload could not be read: ${error.message}`);
}

try {
  for (const guard of [gitWrite, frozenInput, traceReadonly, secretsBoundary, delegationGate]) {
    const verdict = guard(payload);
    if (verdict) deny(verdict.rule, verdict.guard, verdict.why);
  }
} catch (error) {
  deny('G-13', 'entry-point', `a guard threw and could not evaluate: ${error.message}`);
}

process.exit(0);
```

**Two failure modes this skeleton exists to prevent**, both of which have happened:

- **The wrong exit code.** An ordinary error exit is non-blocking; the call proceeds and every boundary is open for that call. Prove the block by attempting a denied action and watching it refuse — do not infer it from the code reading correctly.
- **The path comparison that fails open.** Compare paths through **one shared resolver**, tested against a **captured payload** (`INC-14`). The payload carries an **absolute** path, which cannot start with a repository-relative prefix; and on a case-insensitive filesystem a comparison that does not fold case puts a differently-cased spelling of a protected directory outside its own boundary. Both of these shipped, both failed open, and both had green batteries built entirely from written-down assumptions.

### 22.5 · The trace writer

One script, registered on every event listed in §22.3. It writes one line per event to a per-run file under the gitignored runs directory, and it is the **only** thing that ever writes there (`H-03`).

Implement the payload shape as a **pure function** with the captured shapes asserted in its tests, not inline in the hook — the field names are a contract the runtime owns both ends of, and one of them differed from the documentation in a way that made every result record as zero bytes while looking healthy (§16).
...
### 22.6 · Roles and procedures

```text
.claude/agents/implementer.md          .claude/skills/work-item/SKILL.md
.claude/agents/test-engineer.md        .claude/skills/wrap-up/SKILL.md
.claude/agents/adversarial-auditor.md  .claude/skills/evaluate-harness/SKILL.md
.claude/agents/researcher.md
.claude/agents/harness-evaluator.md
```

Role frontmatter is §13's contract; the `tools` list is what proves write-capability to the delegation gate (`H-05`), so it is never omitted. Procedure frontmatter and bodies are §9 verbatim — including `disable-model-invocation: true`, which is what keeps a procedure a deliberate act rather than something the model triggers on a typo.

**A roster check enforces the six posture dimensions as a property** and fails on a role named `orchestrator` (`G-09`).

### 22.7 · The smoke test — step 5, and the only part that produces evidence
...
5  delegate a write-capable role with       must be DENIED, naming H-05
   no approved spec
6  neuter one guard, run its battery        the battery must FAIL. Restore it
7  corrupt the guard config, retry line 1   must still be DENIED — fail closed (G-13)
8  start a fresh session                    the rules-loaded event appears in the trace
9  run one delegated role end to end        report and trace agree; the run writes a footer
10 run the gate on the deep profile         passes, and the headline names the profile
```

**Line 7 is the one people skip and the one that caught a total failure.** A guard suite that passes every happy path and opens every boundary the moment its own config is unreadable is `INC-12`, and nothing except this line finds it.

**Line 9 is what turns the trace from a feature into evidence.** Until one real delegated run has been corroborated, the trace is a claim about itself.

### 22.8 · Validation checklist
...
- [ ] Every hard rule has **both** a deny rule and a hook dispatch, and none rests on an "ask" rule (`G-03`).
...
- [ ] Every role declares all six posture dimensions and its exact tool list (`G-05`).
- [ ] The trace checker asserts the **hooks are registered**, not merely that trace files parse (§16).
